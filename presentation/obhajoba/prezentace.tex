...
\end{frame}
    ...
    \begin{frame}[fragile]{Klasická negace pro kódování robustnosti}
        Perceptron
...
1 { on(0, K); -on(0, K) } 1 :- not inpfix(K), layer(0, N), K=1..N.
\end{lstlisting}
    \end{frame}

    \begin{frame}[fragile]{Klasická negace pro kódování robustnosti}
        Oblast vstupů --- hammingova vzdálenost
\begin{lstlisting}[language=Prolog, numbers=none]
{ on(0, K) } :- layer(0, N), K=1..N.
...
\end{lstlisting}
        \hrule
\begin{lstlisting}[language=Prolog, numbers=none]
1 { on(0, K); -on(0, K) } 1 :- layer(0, N), K=1..N.
:- #count{ N :  on(0, N), not input(N);
           N : -on(0, N),     input(N) } > R,
    hammdist(R).
\end{lstlisting}
...
\begin{frame}{Hammingova vzdálenost}
\begin{figure}[p]%
    \begin{tikzpicture}[scale=1]
        \begin{loglogaxis}[
                legend entries={M1$\neg$, M1, M2$\neg$, M2, M3$\neg$, M3},
...
\begin{frame}{Fixní hodnoty pozic}
\begin{figure}[p]%
    \begin{tikzpicture}[scale=1]
        \begin{loglogaxis}[
                legend entries={M1$\neg$, M1, M2$\neg$, M2, M3$\neg$, M3},
...
\begin{frame}{Další možnosti rozšíření}
    \begin{itemize}
        \item Zlepšení kódování
        \item Celočíselný vektor vah $\vec w$
    \end{itemize}
\end{frame}
